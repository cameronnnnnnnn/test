//+------------------------------------------------------------------+
//|                                              NAS100_ORB_95.mq5    |
//|   FTMO $15k 1-Step "95% static-floor" config — SEPARATE EA.       |
...
//|     • RiskPercent 1.0%.                                            |
//|   Verified ~95.9% pass on FTMO's static $13,500 floor (block-boot |
...
input double   RiskPercent      = 1.0;     // % risked per trade (1R)
...
input group "=== Misc ==="
input ulong    MagicBase        = 7800000; // DIFFERENT base from the sprint EA (7700000)
...
int OnInit()
{
   ...
   PrintFormat("NAS100_ORB_95: %d sessions, stop=%.0f risk=%.2f%% regime[%.2f,%.2f] partial %.0f%%@%.1fR",
...
void OnTick()
{
   ...
   for(int i=0;i<g_nSess;i++){
      ...
      if(ask>=g_rHigh[i] && (AllowOpposing || !OpposingInProfit(+1))){
         ...
         if(trade.Buy(lots,_Symbol,0.0,sl,tp,"ORB95_"+IntegerToString(g_hours[i]))){ g_traded[i]=true; g_extreme[i]=bid; }
         ...
         if(trade.Sell(lots,_Symbol,0.0,sl,tp,"ORB95_"+IntegerToString(g_hours[i]))){ g_traded[i]=true; g_extreme[i]=ask; }
      }
   }
}
//+------------------------------------------------------------------+
